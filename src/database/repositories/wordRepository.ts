import { Database, queryAll, queryFirst, execute, SQLArgs } from '../helpers';
...
export class WordRepository {
  ...
  async search(query: string): Promise<Word[]> {
    const rows = await queryAll<WordRow>(
      this.db,
      `SELECT * FROM words WHERE base_form LIKE ? OR jp_meaning LIKE ? ORDER BY base_form ASC`,
      [`%${query}%`, `%${query}%`]
    );
    return this.attachConjugations(rows);
  }
  ...
  async findByDateRange(from: string, to: string): Promise<Word[]> {
    const rows = await queryAll<WordRow>(
      this.db,
      `SELECT * FROM words WHERE created_at >= ? AND created_at <= ? ORDER BY created_at DESC`,
      [from, to + ' 23:59:59']
    );
    return this.attachConjugations(rows);
  }
  ...
  async countByDateRange(from: string, to: string): Promise<number> {
    const row = await queryFirst<{ count: number }>(
      this.db,
      `SELECT COUNT(*) as count FROM words WHERE created_at >= ? AND created_at <= ?`,
      [from, to + ' 23:59:59']
    );
    return row?.count ?? 0;
  }
  ...
  async findDueForReview(): Promise<Word[]> {
    ...
    const rows = await queryAll<WordRow>(
      this.db,
      `SELECT * FROM words WHERE next_review IS NULL OR next_review <= ? ORDER BY next_review ASC`,
      [now]
    );
    return this.attachConjugations(rows);
  }
  ...
  async insert(draft: WordDraft): Promise<Word> {
    const result = await execute(
      this.db,
      `INSERT INTO words (base_form, part_of_speech, phonetic_text, phonetic_audio, jp_meaning, example)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        draft.baseForm,
        draft.partOfSpeech,
  ...
  async updateStats(id: number, isCorrect: boolean, nextReview: string): Promise<void> {
    if (isCorrect) {
      await execute(
        this.db,
        `UPDATE words SET times_correct = times_correct + 1,
           last_studied = datetime('now'), next_review = ? WHERE id = ?`,
        [nextReview, id]
      );
    } else {
      await execute(
  ...
  async update(id: number, updates: Partial<WordDraft>): Promise<void> {
    const sets: string[] = [];
    const values: SQLArgs = [];

    if (updates.baseForm !== undefined) { sets.push('base_form = ?'); values.push(updates.baseForm); }
    if (updates.partOfSpeech !== undefined) { sets.push('part_of_speech = ?'); values.push(updates.partOfSpeech); }
    ...
    if (updates.japaneseMeaning !== undefined) { sets.push('jp_meaning = ?'); values.push(updates.japaneseMeaning); }
    if (updates.exampleSentence !== undefined) { sets.push('example = ?'); values.push(updates.exampleSentence ?? null); }

    if (sets.length === 0) return;
    values.push(id);
    await execute(this.db, `UPDATE words SET ${sets.join(', ')} WHERE id = ?`, values);
  }

  async delete(id: number): Promise<void> {
    await execute(this.db, 'DELETE FROM words WHERE id = ?', [id]);
  }
  ...
  private async attachConjugations(rows: WordRow[]): Promise<Word[]> {
    ...
    return Promise.all(
      rows.map(async row => ({
        ...rowToWord(row),
        conjugations: await conjRepo.findByWordId(row.id),
      }))
    );
  }
}
